...
public:
  explicit Node1() : Node("node1") {
    ...
    group2_ = this->create_callback_group(
      rclcpp::CallbackGroupType::Reentrant);
    group3_ = this->create_callback_group(
      rclcpp::CallbackGroupType::Reentrant);

    timer1_ = create_wall_timer(TimerPeriod,
        std::bind(&Node1::callbackTimer1, this), group1_);
    timer2_ = create_wall_timer(TimerPeriod,
        std::bind(&Node1::callbackTimer2, this), group2_);
    timer3_ = create_wall_timer(TimerPeriod,
        std::bind(&Node1::callbackTimer3, this), group3_);
  }
...
public:
  explicit Node2() : Node("node2") {
    group4_ = this->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
    group5_ = this->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);

    timer4_ = create_wall_timer(TimerPeriod,
        std::bind(&Node2::callbackTimer4, this), group4_);
    timer5_ = create_wall_timer(TimerPeriod,
        std::bind(&Node2::callbackTimer5, this), group5_);
  }
...
private:
  ...
  rclcpp::CallbackGroup::SharedPtr group4_;
  rclcpp::CallbackGroup::SharedPtr group5_;
  size_t counter_ = 0;
};
